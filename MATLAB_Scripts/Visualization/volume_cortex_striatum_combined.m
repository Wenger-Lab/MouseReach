...
trad_M = readtable(''); trad_M = sortrows(trad_M,["Day","Mouse","Hand"]); trad_M(trad_M.Mouse == 34,:) = []; %traditional table
...
'PrimarySomatosensoryAreaUpperLimb','fiberTracts','x_maskvol'};

%M volume percentages
%M_volumes = readtable('/home/nikolaus/Desktop/Matlab_Scripts/Lesion Volumes/MCAO2019_volPercBrain.xlsx');
M_volumes = readtable('/home/nikolaus/Desktop/Matlab_Scripts/Lesion Volumes/MCAO2019_percOverlapp_region_new.xlsx'); %percentage overlap 
...
M_volumes = M_volumes(:,areas); M_volumes.Properties.VariableNames(end) = {'Total'}; %rename masked volume into total volume
...
M_volumes(:,2:end) = array2table(M_volumes{:,2:end}.*2); %correct values to one hemisphere only
M_volumes = movevars(M_volumes,'SensorymotorAreas','Before','Total');

M_volumes(M_volumes.Mouse == 34,:) = []; %exclude mouse 34 because no reaches

%PT volume percentages
%PT_volumes = readtable('/home/nikolaus/Desktop/Matlab_Scripts/Lesion Volumes/PT2022_volPercBrain.xlsx'); %total volume percentage
PT_volumes = readtable('/home/nikolaus/Desktop/Matlab_Scripts/Lesion Volumes/PT2022_percOverlapp_region.xlsx');
...
PT_volumes(:,2:end) = array2table(PT_volumes{:,2:end}.*2); %correct values to one hemisphere only
...
%define variables 
%M_vol = M_volumes.ICPerc;
M_vol = M_volumes.Striatum; %Isocortex, Striatum
%PT_vol = PT_volumes.ICPerc; 
...
ylabel('% of occluded striatum','FontSize',axes_font); xlabel({'Stroke type'},'FontSize',axes_font); yticks([10 30 50 70]);
...
xlabel('% of lesioned striatum','FontSize',axes_font);
...
%ylim([-1 4]); yticks([0 2 4]); %d7
%ylim([-0.5 2]); yticks([0 1 2]); %d21

%adjust axis for normalized duration
%ylim([0.75 1.3]); yticks([0.8 1 1.2]); %d21
%ylim([0.85 1.15]); yticks([0.9 1 1.1]); %d7
...
if n == 1, figname = "striatal_occlusion"; else, figname = 'corr_'+string(n); end
